...
| C1 | AI disclosure; "am I talking to a robot?" handled gracefully | 🔶 Adapting | **Policy (team decision 07-18): disclose-only-if-asked** — competence-first opener; the moment anyone asks, Alex confirms plainly and never denies (hard rule, config `never_deny_ai`). Trade-off vs. proactive-disclosure norms owned openly on a deck slide; showcased on camera (montage call 2). |
...
- **Disclosure mode: only-if-asked** (team decision 07-18, config `disclosure.mode`): competence-first opener (account + purpose); the moment anyone asks "is this an AI/robot?", Alex confirms plainly — never denies, never dodges twice. Rationale: the Luo et al. 2019 proactive-disclosure penalty (>79.7% in outbound sales); the trade-off vs. proactive-disclosure norms (TCPA/SB1001 lean) is owned openly on a deck slide, and `mode: early` remains one config line away.
