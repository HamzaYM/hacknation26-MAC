...
1. **CMS Medicare fee schedule** (PFS/OPPS): download CSV → `transform.py` → `benchmarks` rows for the demo CPT set (rate localized to NC).
2. **Hospital price-transparency MRFs**: pull 2–3 real NC hospitals' machine-readable files → extract cash price + negotiated median for the demo CPTs. Real, citable, and the single most confrontational number the agent can say out loud ("your own posted cash price is…"). *(Web-verified 2026-07: build on **Atrium** — per-facility CSVs at static Azure URLs, index at atriumhealth.org/…/pricing — and **Novant** — JSON, file index at www2.novanthealth.org/Public_Files/regulatory/cms-hpt.txt; UNC/Duke publish but are 403-walled or vendor-gated — stretch only.)*
...
- **Benchmark reconciliation (must hold exactly):** Medicare total for demo codes **$438** → **ask target $657–$876** (§8.1's 150–200% ask target; the §10.2 fair-price *band* at 150–250% is $657–$1,095); Mercy MRF cash price **$1,890**; negotiated median ~$2,600. The §14 arc ($4,287 → $3,875 → $2,400 → **$1,650**) sits provably inside these rails: below MRF cash → settlement defensible as "fair," and −62% for Maya.
- **Provenance & provisionality:** the demo CPT list is currently the five codes in `data/seed/` (99283, 71046, 80053, 85025, 96374); the $438/$1,890 totals are v0 placeholders shaped like real output. J replaces values with real NC-localized CMS/MRF rates before the H3 CPT-list freeze — updating `demo_answer_key.json`, this section, and §14 *together*, never one alone. "Mercy General" is fictional: its benchmark row is synthetic, engineered to the reconciliation values; the real NC hospital MRF pulls prove the pipeline and methodology, and the UI labels Mercy's figures as demo data.
...
- **0:55–1:15 · War Room.** Dossier renders: three armed levers with real numbers (Medicare **$438** · Mercy's own posted cash price **$1,890** · §501(r) + 250% FPL at a nonprofit). Three call cards queue: Facility · ER Physician Group · Collections.
