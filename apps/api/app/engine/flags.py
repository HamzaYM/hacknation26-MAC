...
PTP pairs) → phantom → nsa_balance_billing → denial → units_error →
...
on them — see apps/api/tests/test_flags.py for the demo arithmetic):
  ...
  nsa_balance_billing  patient_balance − EOB patient_responsibility_total
...
def detect_flags(
    ...
    *,
    ...
    """All red flags for a JobSpec, in stable detector order.
    ...
    # ── nsa_balance_billing: in-network (or emergency) claim where the provider
    #    bills the patient above the EOB patient-responsibility figure. ───────
    nsa_cfg = rf.get("nsa_balance_billing", {})
    ins = job_spec.insurance or {}
    network = ins.get("network_status")
    emergency = bool(ins.get("emergency_services") or ins.get("emergency"))
    protected = (network == "in_network") or (
        emergency and nsa_cfg.get("emergency_always_protected", True)
    )
    ...
    if protected and eob_present and balance is not None and eob_resp is not None:
        overage = round(balance - eob_resp, 2)
        if overage > nsa_cfg.get("tolerance_usd", 1.0):
            do_not_negotiate = config["thresholds"].get("nsa_do_not_negotiate", {})
            flags.append(DerivedFlag(
                type="nsa_balance_billing",
                cpt=None,
                evidence={
