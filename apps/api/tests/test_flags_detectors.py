...
nsa (generalized path), denial, units_error, absent_from_chargemaster, and the
...
# ── nsa (generalized balance-billing path — emits the unified "nsa" type) ────
# Integration note: WS2's detector was unified with #67's marker-based detector
# onto a single flag type string "nsa" (existing tests + consumers assert it).
# The generalized reconciliation-driven logic below is preserved as-is; only the
# emitted type name changed from "nsa_balance_billing" to "nsa".
...
def test_nsa_no_fire_for_out_of_network_non_emergency():
    ...
    assert "nsa" not in _types(flags)
