...
| Floor / target / anchor | $1,700 / $876 / $657 — $1,650 is under Maya's $1,700 ceiling, but above the engine's $876 target, so a human signs off (`escalation_required`) |
...
| **A5** 0:38–0:51 | The supervisor call: ticker **$4,287 → $3,875 → $2,400 → $1,650**, lever chips arming *before* each drop; PLAY AUDIO of the duplicate cite ("code 71046 appears twice…"); flash the `escalation_required` event at $1,650. | "Duplicate X-ray removed. Medicare and the hospital's own posted cash price cited. $4,287 becomes $1,650 — sixty-two percent off — and because that's under Maya's $1,700 ceiling, but above the engine's $876 target, a human signs off on the final number." | **S4** (+S6) |
...
Audit items now **obsolete** (done since the audit): intake voice widget + live PDF vision parse (audit #3 — built, verified with exact reconciliation and all 4 flags on real PDFs), report transcript-evidence + audio slots (audit #4), Supabase login, public deployment at hagglfor.me, locked-numbers sweep (G5 — all frontend files reconciled to demo_answer_key.json).
...
| **G3** | **Compute the honesty audit** — **DONE**: `app/engine/honesty.py` (3 checks: disclosure, robot-question, number-tracing) runs on every post-call webhook transcript; result stored as `honesty_audit` event. 10 tests. Simulated calls still carry pre-staged audits (same logic, verified consistent). | Converts C3/S5 from claimed to verified — the "honest about the hard parts" differentiator. | ~~Claude~~ | ✅ |
...
| ~~G5~~ | ~~Locked-numbers sweep~~ — **DONE** (all "$1,890" → "$2,633", "Carolina Emergency Physicians" → "Bay State Emergency Physicians" across bills detail, action items, procedures, and mock prompts; "Charlotte NC" → "Boston MA" in mock-generation-prompt) | ~~Judges who diff the report against the call WILL catch drift.~~ | ~~Claude~~ | ✅ |
| **G6** | **Config-swap proof on camera** (audit #8): `moving.yaml` exists; `VERTICAL` env wire **DONE** (`VERTICAL=moving` → app loads moving config). Remaining: film the yaml diff (shot B7). | Hits the brief's explicit "swap a config file, not rewrite your agents" line. | Kar Shin (film) | ✅ code · film TBD |
...
10. **"Why trust the −62%?"** Every step is traceable: $4,287 → $3,875 (duplicate 71046, $412) → $2,400 (Medicare $438 + posted cash $2,633.25 cited) → $1,650 settled, ref MG-ADJ-2247 — and because $1,650 is under Maya's $1,700 ceiling, but above the engine's $876 target, the engine forced a human approval before accepting.
